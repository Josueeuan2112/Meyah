-- Add candidato_id to job_applicants_near return columns (needed for chat initiation)
-- Must DROP + RECREATE because RETURNS TABLE signature changes.
...
AS $$
  ...
  SELECT id, candidato_id, estado, mensaje, created_at, viewed_at,
         candidato_nombre, candidato_phone, distancia_m
  FROM base
